...
if echo "$REGISTER_RESPONSE" | grep -q "error"; then
  echo "⚠️  Registration may have failed or user already exists: $REGISTER_RESPONSE"
else
...
SCHEMA_RESPONSE=$(curl -s -X POST ${API_BASE}/apis/${API_ID}/import-schema \
  ...
  -H "Authorization: Bearer $TOKEN" \
  -d "{
    \"schemaContent\": \"$(cat /Users/frane/workspace/apifai/test-schema.json | jq -c . | sed 's/\"/\\\"/g')\",
...
echo "📊 Import results:"
echo "$SCHEMA_RESPONSE" | jq -r '.operations as $ops | .resources as $res | "- Operations: \($ops | length) - Resources: \($res | length)"' 2>/dev/null || echo "- Response: $SCHEMA_RESPONSE"
...
if [ "$TOOL_COUNT" -gt "0" ]; then
  ...
  echo "🛠️  Tool details:"
  echo "$TOOLS_RESPONSE" | jq -r '.data[] | "- \(.name): \(.description)"' 2>/dev/null || echo "$TOOLS_RESPONSE"
else
  echo "⚠️  No tools found in response: $TOOLS_RESPONSE"
fi
