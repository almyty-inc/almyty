...
- **Never-synced providers, at boot and before each sweep.** Every active provider whose `modelsSyncedAt` is still null (it has never imported a list) gets its key checked and its list imported, so its models are usable without anyone connecting it again (`CatalogWarmupService`). The job runs in the queue worker, so it never holds up startup; it has a stable job id, and a Postgres session advisory lock lets one instance do the work while the others return. Vendors are worked a few at a time, one provider of a vendor after another. A refused key leaves the models unusable (see Readiness); an outage changes nothing, and the provider is tried again before the next sweep. Off under `NODE_ENV=test` and `MODEL_CATALOG_BACKFILL=off`, which also switches the sweep off.
- **On an empty list.** When `GET /models` (what the Models page and the model picker read) has no usable model to show and the org has providers, it runs the same check and sync for those providers and waits a few seconds for it before answering. Each provider is checked this way at most once every ten minutes, across every instance (a Redis claim shared with the boot sync).
