...
**Readiness, as built (2026-09-25).** The per-card validation run in (d) is replaced at the model level by the provider's key check: connecting a provider checks its key with one real call, and every model the provider lists becomes selectable together when that check passes. A key the vendor later refuses takes them back out; a model the vendor answers MODEL_NOT_FOUND for is marked unavailable on its own; a model the provider stops listing is retired by the next sync. A card with no provider row (an endpoint) still needs its own passing call, which the reconcile loop makes by itself when a model on the customer's cloud reaches ready. The adapter half of the rule (registered adapter + passing conformance run) is unchanged. Current description: `docs/models.md`, Readiness.
...
2. ModelVersion (model_versions): registryUri (hf://org/repo@sha | s3://bucket/prefix@etag | file:// for runner-local), base, sizeBytes, quantizations string[], lineage jsonb { trainingJobId?, datasetRef?, parentVersionId? }, evalScores jsonb (interim), manifestSha. Registry format: plain safetensors + almyty-manifest.json (schema in docs/model-registry.md: base, tokenizer ref, chat template, license, created, lineage). CORRECTED 2026-09-09: a Hugging Face repository is the usual source because it is what the providers natively import; our own S3-compatible registry is for the providers that read object storage natively (AWS) and for a self-host, and is optional everywhere else.
...
AdapterCapabilities: architectures string[]|'any'; lora merged|multi|none; serverless; dedicated; scaleToZero; regions; registrySources ('hub'|'s3'|'gcs'|'local')[] native default first.
Rules: an adapter may not import or invoke another adapter; registrySources names what the provider can really read, most preferred first, and the first entry is the native default (CORRECTED 2026-09-09: the original rule here said every adapter must accept registrySources ['s3'] alone. That was wrong. almyty supports inference through the providers, and most providers import from a Hugging Face repository, not from our object storage; the rule forced a workaround per provider, including one adapter streaming weight bytes through this backend. Weight files never pass through almyty, and an adapter refuses a source its provider cannot read); UI warnings derive from capabilities() (architecture mismatch blocks at submit, not at provider error).
...
1. Hand-registered custom OpenAI-compatible endpoint; router selects it under a privacy/cost policy; run audit shows model + version + rationale.
   - [x] A `custom` provider connected with `POST /llm-providers/connect` gives its listed models, and its passing key check makes them selectable (the readiness rule; previously one validation run per card): `modules/llm-providers/__tests__/connect-provider.spec.ts`, `modules/model-catalog/__tests__/model-catalog.service.spec.ts`
...
## Provider API deltas (verified 2026-09-08 against current docs)

- **Hugging Face Inference Endpoints**: a plain REST API, base `https://api.endpoints.huggingface.cloud/v2/endpoint/{namespace}`, bearer token. Create is `POST` with `name`, `type` (public|authenticated|private), `provider {vendor, region}`, `compute {accelerator, instanceType, instanceSize, scaling {minReplica, maxReplica, scaleToZeroTimeout}}`, `model {repository, revision?, framework, image, task}`; `GET/PUT/DELETE /{name}`, `POST /{name}/pause|resume|scale-to-zero`; status `state` in pending|initializing|updating|updateFailed|running|paused|failed|scaledToZero with `url`, `readyReplica`, `targetReplica`, `message`. Delta from the spec: the API requires a Hub `model.repository` even for a custom image. For the S3 registry source the adapter therefore deploys `framework: custom` with a vLLM image whose environment points at the registry URI (with the registry's read credentials passed as endpoint secrets); `repository` carries a placeholder the container ignores. Hub source uses `repository`/`revision` directly.
- **Modal**: there is no HTTP API for deploying or stopping apps; deployment is `modal deploy` (CLI, Python) and teardown is `modal app stop APP -y`; `modal app list --json` lists apps. Web endpoint URLs are deterministic: `https://<workspace>[-<env>]--<app>-<function>.modal.run`, or a custom `label`. Delta from the spec: the Modal adapter drives the `modal` CLI in a subprocess with a generated vLLM app file and the workspace token in the environment, so the API container must ship the `modal` package. Cost: Modal exposes no billing API; `costSnapshot` reports the rate from the chosen GPU price table on the card and spend as rate times observed running time, marked as an estimate.
