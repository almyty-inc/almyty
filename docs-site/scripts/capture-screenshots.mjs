...
const steps = [
  ...
  {
    id: 'analytics-tools',
    title: 'Analytics → Tools tab',
    auth: true,
    navigate: async (page) => page.goto(`${BASE_URL}/analytics`),
    waitFor: async (page) => page.getByRole('heading', { name: /analytics/i }).waitFor(),
    after: async (page) => page.getByRole('tab', { name: /^tools$/i }).click().catch(() => null),
  },
  {
    id: 'analytics-gateways',
    title: 'Analytics → Gateways tab',
    auth: true,
    navigate: async (page) => page.goto(`${BASE_URL}/analytics`),
    waitFor: async (page) => page.getByRole('heading', { name: /analytics/i }).waitFor(),
    after: async (page) => page.getByRole('tab', { name: /^gateways$/i }).click().catch(() => null),
  },
  {
    id: 'analytics-agents',
    title: 'Analytics → Agents tab',
    auth: true,
    navigate: async (page) => page.goto(`${BASE_URL}/analytics`),
    waitFor: async (page) => page.getByRole('heading', { name: /analytics/i }).waitFor(),
    after: async (page) => page.getByRole('tab', { name: /^agents$/i }).click().catch(() => null),
  },
  {
    id: 'analytics-audit',
