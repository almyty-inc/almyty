...
1. Open **Tools → Create tool** (`/tools/new`) and select **Model prompt**.
2. Give it a **name** and **description**.
3. Choose a model with the shared model picker: one search box over every available model. Connect a provider under **Models → Connect a provider** first if needed.
...
9. Click **Create tool**, then test it from the tool's detail page.
...
| **Anthropic** | claude-sonnet-5, claude-opus-5, claude-haiku-4-5, etc. |
| **Custom** | Any OpenAI-compatible API endpoint |

Connect a provider in **Models → Connect a provider** before creating LLM tools.
