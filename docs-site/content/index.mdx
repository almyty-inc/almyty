...
Open **APIs → Connect API**, give it a name, choose its type, and enter its base URL. After connecting a schema-based API, import its schema from a URL, file, or pasted content. You can also reopen **Import Schema** from the API's actions menu. Leave **Auto-generate Tools** enabled. OpenAPI, GraphQL, SOAP, Protobuf, and npm SDKs are supported; the setup fields depend on the type.

![Import an OpenAPI schema from a URL](/screenshots/apis-import-dialog.png)
