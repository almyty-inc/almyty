...
Point your assistant at the gateway and your API is live inside it. Grab the endpoint from the gateway's **Integrations** tab (it looks like `https://api.almyty.com/your-org/billing`) and a key from its **Authentication** panel. Then, in Claude Code:

```bash
claude mcp add billing --transport http \
  --header "x-api-key: gw_live_..." \
  https://api.almyty.com/your-org/billing
```
...
```bash
npx @almyty/mcp-server your-org/billing
```
