...
A channel is therefore not usable until its verification secret is set, and the surface node in the publish canvas names the exact key it is waiting for. If a channel is not receiving messages, that key is the first thing to check.
