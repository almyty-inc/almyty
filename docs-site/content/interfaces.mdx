...
3. The channel's connection test performs a real authentication probe against the platform, so a bad token surfaces immediately instead of on the first missed message.
