...
That is worth stating plainly because it is a change in behaviour: the secret used to be optional, and a channel without one quietly accepted any request that reached its URL. If a channel of yours stops receiving messages after upgrading, a missing secret is the first thing to check. The surface node in the publish canvas names the exact key it is waiting for.
...
- A rejected inbound message is recorded as a failed channel event with the reason, so a wrong secret looks different from a quiet channel.
