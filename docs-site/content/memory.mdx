...
Memory gives an agent recall across runs. It stores facts, preferences, context,
past episodes, and standing instructions, and pulls the relevant ones back into
the prompt by **semantic (embedding) search** — so the agent remembers what
matters without you restating it every time.

Memory is distinct from [constraints](/agents/autonomous#constraints):
constraints are always-on prompt rules that gate behavior; memory is retrieved
per turn by relevance.
