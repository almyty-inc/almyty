...
With the entitlement enabled, **Settings → Advanced → Compliance** shows the effective policy and posture report. This demo organization is using the secure default policy, not a custom configuration.

![Enabled compliance settings showing the default policy and posture report](/screenshots/settings-compliance-enabled.png)

Set the policy under **Settings → Advanced → Compliance** (admin/owner only). It declares which built-in guardrails are enforced org-wide and how they behave:
