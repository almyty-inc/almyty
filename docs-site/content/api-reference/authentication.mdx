...
4. Pick a name, select the gateway or agent to scope it to, and choose permissions
...
```bash
curl /apis -H "Authorization: Bearer ak_live_abc123..."
```

For CLI tools, run `npx @almyty/auth login` instead — it opens your browser, authenticates, and stores the key at `~/.almyty/credentials.json` so every CLI picks it up automatically.
