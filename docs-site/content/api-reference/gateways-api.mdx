...
![Gateway detail in the UI](/screenshots/gateway-detail.png)
...
| `type` | string | Yes | `tools` (MCP, UTCP and Skills from one address), `mcp`, `utcp`, `skills`, `a2a`. A web chat or messaging channel is a place on an app, and is refused here (`APP_SURFACE_NEEDS_APP`) |
