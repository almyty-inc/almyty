...
| **Art. 14 — human oversight**: humans must be able to intervene | Built-in `request_approval` tool pauses any run for human sign-off; approval policies (multi-step, conditional, quorum) let you require multiple reviewers for sensitive actions | [Approvals](/approvals) |
...
| **Art. 9 — risk management** | Guardrail constraints on agents, the pii-filter and security-scanner plugins, and (Enterprise) the compliance pack that enforces them org-wide with a posture report | Agent constraints, [Plugins](/self-hosting) |
| **Art. 26 — deployer duties** (use per instructions, monitor, retain logs) | Usage monitoring, spend governance, per-run traces, and audit retention/export (Enterprise) give you the operational evidence | [Analytics](/analytics) |
...
3. Configure `request_approval` (and approval policies) for consequential actions.
