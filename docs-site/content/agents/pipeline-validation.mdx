...
A broken pipeline never reaches production: almyty validates every workflow agent
before it can be activated or invoked. Validation runs automatically when you
save in the builder and surfaces errors directly on the canvas, so you fix them
in place.

![Validation errors on the canvas](/screenshots/agent-builder.png)
...
Validation happens automatically on save, whether you save from the builder or
update the agent through the API. When a pipeline fails, the response lists each
problem by node and field:
