...
expression pulls a value from the invocation input or from another node's output,
using `{{...}}` syntax, resolved at runtime just before each node executes. Type
`{{` in any node config field and the builder autocompletes the available
sources.
