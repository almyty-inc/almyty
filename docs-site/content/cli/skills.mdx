...
![@almyty/skills CLI help](/screenshots/cli/skills-help.png)
...
|--------|---------|-------------|
| `@org/gateway` | `@acme/billing` | All skills from the `billing` gateway in the `acme` org |
| `@org/gateway/skill` | `@acme/billing/get-invoice` | A single skill |
| `<name>` | `get-invoice` | Search by name across all gateways |
...
Your gateways:
  ...
  Billing API
    Type: skills
    Use:  npx @almyty/skills install @acme/billing
...
```
.claude/skills/get-invoice/SKILL.md
.claude/skills/list-pets/SKILL.md
.agents/skills/get-invoice/SKILL.md
.agents/skills/list-pets/SKILL.md
```
...
Only skills almyty installed are removed, identified by the `author: almyty` line in their SKILL.md frontmatter. Your other skills are untouched.
...
```bash
$ npx @almyty/skills watch @acme/billing
Watching Billing API (every 60s)
Syncing to 2 agent target(s):
  Claude Code: .claude/skills
  Universal (.agents/skills): .agents/skills
