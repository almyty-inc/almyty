...
The MCP server turns your almyty gateway into an MCP-compatible tool provider. Add it to any MCP client:
...
{
  "mcpServers": {
    "almyty": { "command": "npx", "args": ["-y", "@almyty/mcp-server"] }
  }
}
```

Set `ALMYTY_MODE=full` to register every tool individually (higher token overhead) instead of the default skill-first mode.

## Testing

Smoke tests for all CLI commands live in `packages/cli-tests/`:

```bash
$ cd packages/cli-tests
$ RUN_CLI_SMOKE=1 npx vitest run   # requires auth + a gateway with tools
```
