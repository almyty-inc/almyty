...
4. Set `ALMYTY_GATEWAY_ID` to your gateway's `@org/slug` identifier
...
```bash
$ claude mcp add petstore -- npx -y @almyty/mcp-server \
...
| `ALMYTY_GATEWAY_ID` | Gateway to serve (`@orgSlug/gatewaySlug`) | **Required** |
| `ALMYTY_MODE` | `skill-first` or `full` | `skill-first` |

### Finding your gateway ID

Your gateway ID is the `@org/slug` shown on the gateway detail page in the UI. For example, if your org is "acme" and your gateway slug is "petstore", the ID is `@acme/petstore`.
