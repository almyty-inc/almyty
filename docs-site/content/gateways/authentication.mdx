...
| Endpoint | Purpose |
|----------|---------|
| `POST /acme/billing/register` | Dynamic Client Registration |
| `GET /acme/billing/authorize` | Start the authorization code flow (with `code_challenge`, `code_challenge_method=S256`) |
| `POST /acme/billing/token` | Exchange the code (plus `code_verifier`) for tokens |
