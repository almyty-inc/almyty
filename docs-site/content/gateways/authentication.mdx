...
1. Open a gateway's detail page and find the **Authentication** section
2. Click **Add auth method** to expand the inline form
3. Select a type: API Key, Bearer Token, Basic Auth, JWT, OAuth 2.1, or None
4. Fill in the configuration and click **Add auth method**

To issue API keys, click **Generate key** after adding an API Key method. Name the key in the inline form. The raw key is shown once at creation time, then stored only as a SHA-256 hash. The key API also supports optional expiry, scopes, and `lastUsedAt` tracking.
