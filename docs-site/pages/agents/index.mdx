...
Agents in apifai are composable AI pipelines that orchestrate LLM calls, tool executions,
...
When an agent is invoked, apifai:
...
```bash
curl -X POST https://api.apif.ai/agents \
...
apifai ships with several built-in templates:
...
```bash
curl -X POST https://api.apif.ai/agents/{id}/invoke \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
# Export
curl https://api.apif.ai/agents/{id}/export \
  -H "Authorization: Bearer $TOKEN" > agent.json

# Import
curl -X POST https://api.apif.ai/agents/import \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d @agent.json
```
