...
export function ConditionNode({ data, selected }: NodeProps) {
  return (
    <div className={`rounded-xl border-2 bg-card shadow-sm w-[220px] ${selected ? 'border-primary' : 'border-border'}`}>
      ...
      <div className="px-3 py-1.5 bg-amber-50 dark:bg-amber-950 rounded-t-[10px] border-b flex items-center gap-1.5">
        <GitBranch className="h-3 w-3 text-amber-700 dark:text-amber-300" />
        <span className="text-xs font-medium text-amber-700 dark:text-amber-300">Condition</span>
      </div>
      <div className="p-3">
