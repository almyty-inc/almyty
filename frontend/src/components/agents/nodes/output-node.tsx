...
export function OutputNode({ data, selected }: NodeProps) {
  return (
    <div className={`rounded-xl border-2 bg-card shadow-sm w-[220px] ${selected ? 'border-primary' : 'border-border'}`}>
      ...
      <div className="px-3 py-1.5 bg-red-50 dark:bg-red-950 rounded-t-[10px] border-b flex items-center gap-1.5">
        <LogOut className="h-3 w-3 text-red-700 dark:text-red-300" />
        <span className="text-xs font-medium text-red-700 dark:text-red-300">Output</span>
      </div>
      <div className="p-3">
