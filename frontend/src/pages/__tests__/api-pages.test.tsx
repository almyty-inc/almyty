...
describe('private visibility on the API pages', () => {
  it('sends visibility "private" when a new API is made private', async () => {
    vi.mocked(apisApi.create).mockResolvedValue({ id: 'api-9', name: 'Mine' } as any)
    const user = userEvent.setup()
    renderAt('/apis/new')
    await user.type(await screen.findByLabelText(/API name/), 'Mine')
  ...
  it('starts /apis/:id/edit from the API\'s own private scope and keeps it on save', async () => {
    vi.mocked(apisApi.getById).mockResolvedValue({ ...API, visibility: 'private', teamId: null } as any)
    vi.mocked(apisApi.update).mockResolvedValue({ ...API } as any)
    const user = userEvent.setup()
    renderAt('/apis/api-1/edit')
    await screen.findByLabelText(/API name/)
