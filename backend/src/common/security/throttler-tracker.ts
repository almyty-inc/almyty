 ...
 *   anonymous-surface limiter it cannot skip a scope — and the two other
 *   options are worse: skipping the limit would make "send nothing we
 *   can read" an unthrottled path, and inventing a per-request key is
 *   the same thing spelled differently. A merged bucket throttles harder,
 *   never softer. It is also not reachable by a caller: the ingress
 *   always sets X-Forwarded-For, and a direct connection always has a
 *   socket address, so this only catches a request whose socket is
 *   already gone.
 */
