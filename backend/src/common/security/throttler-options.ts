 ...
 * ever calls is the same bug with better comments.
 * `__tests__/throttler-options.spec.ts` boots a real guard on these
 * options.
 *
 * `getTracker` here is the module-wide default, so it also keys every
 * per-route `@Throttle(...)` override (login, register, forgot-password,
 * agent runs, ...). Those overrides change the limit and the window,
 * not the bucket: under the library default they were platform-wide too,
 * e.g. ten logins per five minutes for everyone.
 */
