...
describe('CredentialRefResolver', () => {
  describe('resolve', () => {
    it('returns the decrypted config and the well-known secrets of an active org credential', async () => {
      ...
      const resolved = await store.resolver.resolve('org-1', row.id, { principal: null, context: { purpose: 'spec' } });
    ...
    it('decrypts nested maps one level down (MCP custom headers)', async () => {
      ...
      const resolved = await store.resolver.resolve('org-1', row.id, { principal: null });
    ...
    it('throws CREDENTIAL_NOT_FOUND for a missing row and for a row of another org', async () => {
      ...
      await expect(store.resolver.resolve('org-1', 'nope', { principal: null })).rejects.toMatchObject({
        response: { code: 'CREDENTIAL_NOT_FOUND' },
      });
      await expect(store.resolver.resolve('org-1', other.id, { principal: null })).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('throws CREDENTIAL_INACTIVE and CREDENTIAL_EXPIRED', async () => {
      ...
      await expect(store.resolver.resolve('org-1', inactive.id, { principal: null })).rejects.toMatchObject({ response: { code: 'CREDENTIAL_INACTIVE' } });
      await expect(store.resolver.resolve('org-1', expired.id, { principal: null })).rejects.toMatchObject({ response: { code: 'CREDENTIAL_EXPIRED' } });
    });
    ...
    it('usePolicy() swaps the policy at runtime (the gate 2 seam)', async () => {
      const store = makeCredentialRefFake();
      const row = store.seed({ organizationId: 'org-1', config: { apiKey: encryptField('sk') } });
      await expect(store.resolver.resolve('org-1', row.id, { principal: null })).resolves.toBeDefined();
      store.resolver.usePolicy({ assertCanUse: async () => { throw new ForbiddenException({ code: 'NO_GRANT' }); } });
      await expect(store.resolver.resolve('org-1', row.id, { principal: null })).rejects.toMatchObject({ response: { code: 'NO_GRANT' } });
    });
    ...
    it('tryResolve returns null for a missing or inactive row and rethrows a policy denial', async () => {
      ...
      expect(await store.resolver.tryResolve('org-1', undefined, { principal: null })).toBeNull();
      expect(await store.resolver.tryResolve('org-1', 'missing', { principal: null })).toBeNull();
      expect(await store.resolver.tryResolve('org-1', inactive.id, { principal: null })).toBeNull();
      const row = store.seed({ organizationId: 'org-1', config: { apiKey: 'x' } });
      store.resolver.usePolicy({ assertCanUse: async () => { throw new ForbiddenException({ code: 'NO_GRANT' }); } });
      await expect(store.resolver.tryResolve('org-1', row.id, { principal: null })).rejects.toBeInstanceOf(ForbiddenException);
    });
  });
  ...
  describe('managed rows', () => {
    it('createManaged encrypts every declared secret key, nested maps included, and records the owner', async () => {
      ...
      const resolved = await store.resolver.resolve('org-1', saved.id, { principal: null });
