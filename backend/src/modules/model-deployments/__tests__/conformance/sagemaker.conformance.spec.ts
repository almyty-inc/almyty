 ...
 * SageMaker.{Operation}), the documented EndpointStatus values, the
 * {__type, message} error envelope, and the quirk that a missing endpoint
 * is a 400 ValidationException rather than a 404. Live mode
...
function fixtureHttp() {
    ...
    request: jest.fn(async (config: { method: string; url: string; headers: Record<string, string>; data?: string }) => {
      ...
      switch (target) {
        ...
        case 'UpdateEndpointWeightsAndCapacities': {
          const ep = endpoints.get(body.EndpointName);
          if (!ep) throw missingEndpoint(body.EndpointName);
          const want = body.DesiredWeightsAndCapacities[0];
          ep.ProductionVariants[0].DesiredInstanceCount = want.DesiredInstanceCount;
          ep.EndpointStatus = 'Updating';
          return { status: 200, data: { EndpointArn: ep.EndpointArn } };
        }
        case 'DeleteEndpoint':
  ...
  return { models, configs, endpoints, calls, http };
}
...
describe('sagemaker request shape', () => {
  ...
  it('creates model, endpoint config and endpoint in order with the registry prefix as uncompressed model data', async () => {
    const f = fixtureHttp();
    const a = new SageMakerAdapter(f.http);
    const ref = await a.deploy(request(), creds);
    expect(f.calls.map((c) => c.target)).toEqual(['CreateModel', 'CreateEndpointConfig', 'CreateEndpoint']);
    ...
    expect(ref).toMatchObject({ endpointName: 'almyty-abc123', endpointConfigName: 'almyty-abc123-cfg', modelName: 'almyty-abc123-model', region: 'eu-west-1', instanceType: 'ml.g6.2xlarge', initialInstanceCount: 2, hourlyRateCents: 141 });
    expect(ref.url).toBe('https://runtime.sagemaker.eu-west-1.amazonaws.com/endpoints/almyty-abc123/invocations');
    expect(JSON.stringify(f.calls.map((c) => c.body))).not.toContain('secret');
  });

  it('omits managed scaling for a fixed replica count and honours a custom image', async () => {
    const f = fixtureHttp();
    const a = new SageMakerAdapter(f.http);
    await a.deploy(request({ desired: { replicas: 1 }, providerConfig: { region: 'us-east-1', executionRoleArn: 'arn:aws:iam::1:role/r', image: 'vllm/vllm-openai:latest' } }), creds);
    ...
    expect(variant.ManagedInstanceScaling).toBeUndefined();
    expect(variant.RoutingConfig).toBeUndefined();
  ...
  it('maps every documented EndpointStatus, with InService at zero instances as stopped', async () => {
    const f = fixtureHttp();
    const a = new SageMakerAdapter(f.http);
    const ref = await a.deploy(request(), creds);
    const ep = f.endpoints.get(ref.endpointName);
    ep.ProductionVariants[0].CurrentInstanceCount = 2;
    for (const [raw, expected] of Object.entries({ Creating: 'ready', Updating: 'ready', SystemUpdating: 'scaling', RollingBack: 'scaling', InService: 'ready', OutOfService: 'stopped', Deleting: 'stopped', Failed: 'failed', UpdateRollbackFailed: 'failed' })) {
      // Creating and Updating come back as ready because the fixture completes them on read, like a real endpoint that came up.
    ...
    ep.EndpointStatus = 'InService';
    ep.ProductionVariants[0].CurrentInstanceCount = 0;
    expect((await a.readEndpoint(ref, creds)).state).toBe('stopped');
  });

  it('scales the primary variant by instance count and tears down endpoint, config and model, tolerating already-gone resources', async () => {
    const f = fixtureHttp();
    const a = new SageMakerAdapter(f.http);
    const ref = await a.deploy(request(), creds);
    ...
    expect(scale.target).toBe('UpdateEndpointWeightsAndCapacities');
    expect(scale.body).toEqual({ EndpointName: 'almyty-abc123', DesiredWeightsAndCapacities: [{ VariantName: 'primary', DesiredInstanceCount: 3 }] });
    const scaled = await a.readEndpoint(ref, creds);
    expect(scaled.replicas).toBe(3);
    expect((await a.costSnapshot(ref, creds)).ratePerHourCents).toBe(141 * 3);

    await a.teardown(ref, creds);
    expect(f.calls.slice(-3).map((c) => c.target)).toEqual(['DeleteEndpoint', 'DeleteEndpointConfig', 'DeleteModel']);
    expect(f.endpoints.size + f.configs.size + f.models.size).toBe(0);
