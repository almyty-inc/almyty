...
export class ToolsService {
  ...
  async getTools(filters: ToolSearchFilters): Promise<{
    ...
    const queryBuilder = this.toolRepository
      ...
      .leftJoin('gateway_tools', 'gt', 'gt.toolId = tool.id')
      .leftJoinAndSelect('gt.gateway', 'gateway')
  ...
  async createFromOperation(
    ...
    const toolParameters = this.generateToolParametersFromOperation(operationWithApi);
  ...
  private generateToolParametersFromOperation(operation: Operation): Record<string, any> {
    ...
    if (operation.parameters?.body) {
      if (Object.keys(operation.parameters.body).length === 1) {
        // Single body parameter - flatten it
        const bodyParam = Object.values(operation.parameters.body)[0] as any;
        if (bodyParam.properties) {
          Object.entries(bodyParam.properties).forEach(([name, prop]: [string, any]) => {
            parameters.properties[name] = prop;
          });
          if (bodyParam.required) {
            parameters.required.push(...bodyParam.required);
          }
        } else {
          parameters.properties.body = bodyParam;
          if (bodyParam.required) {
