...
export class ToolsService {
  ...
  async createTool(
    ...
    try {
      ...
      // For custom tools, auto-create API record
      let autoApiId: string | null = null;
      if (createToolDto.code && !createToolDto.operationId) {
        try {
          const apiEntity = this.apiRepository.create({
            name: `Custom: ${createToolDto.name}`,
            description: createToolDto.description || 'Custom tool API',
            baseUrl: 'internal://custom',
            type: ApiType.OTHER,
            version: '1.0.0',
            status: 'active' as any,
            organizationId,
          });
          const api = await this.apiRepository.save(apiEntity);
          autoApiId = api.id;
        } catch (err) {
          this.logger.warn(`Failed to auto-create API: ${err.message}`);
        }
      }
