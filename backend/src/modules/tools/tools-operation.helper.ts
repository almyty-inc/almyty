...
export class ToolsOperationHelper {
  ...
  async buildFromOperation(
    ...
    const tool = this.toolRepository.create({
      ...
      generated: true,
      // The API's scope: tools generated from a team API are its team's,
      // from a private API its owner's. Anything wider would publish the
      // API through its tools (generatedToolScope).
      ...generatedToolScope(operationWithApi.api),
      metadata: {
        autoGenerated: true,
        sourceOperation: {
  ...
  async prepareUpdateFromOperation(
    ...
    // A generated tool stays in its API's scope on every regeneration.
    if (tool.generated) applyGeneratedToolScope(tool, operationWithApi.api);
    tool.metadata = {
      ...tool.metadata,
      autoGenerated: true,
      sourceOperation: {
