...
export enum MessageRole {
  ...
  ASSISTANT = 'assistant',
  TOOL = 'tool',
  FUNCTION = 'function', // For OpenAI function calling
}
...
export class LlmMessage {
  ...
  contentParts: MessageContent[]; // For multimodal content

  @Column({ type: 'json', nullable: true })
  toolCalls: ToolCall[];

  @Column({ type: 'json', nullable: true })
  functionCall: FunctionCall; // For OpenAI function calling

  @Column({ nullable: true })
  toolCallId: string; // For tool result messages

  @Column({ nullable: true })
  functionName: string; // For function result messages
  ...
  @Column({ nullable: true })
  finishReason: string; // 'stop', 'length', 'tool_calls', 'function_call', etc.
  ...
  updateMetadata(updates: Partial<LlmMessage['metadata']>): void {
    this.metadata = {
      ...this.metadata,
      ...updates,
    };
  ...
  static createUserMessage(sessionId: string, content: string | MessageContent[]): LlmMessage {
  ...
  static createAssistantMessage(sessionId: string, content: string): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
  ...
  static createToolCallMessage(sessionId: string, toolCalls: ToolCall[]): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
    message.role = MessageRole.ASSISTANT;
  ...
  static createToolResultMessage(sessionId: string, toolCallId: string, result: any, error?: string): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
  ...
  static createFunctionCallMessage(sessionId: string, functionCall: FunctionCall): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
    message.role = MessageRole.ASSISTANT;
  ...
  static createFunctionResultMessage(sessionId: string, functionName: string, result: string, error?: string): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
  ...
  static createSystemMessage(sessionId: string, content: string): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
  ...
  static createErrorMessage(sessionId: string, error: string): LlmMessage {
    const message = new LlmMessage();
    message.sessionId = sessionId;
    message.role = MessageRole.ASSISTANT;
        ...
        if (part.type === 'text') {
          ...
          return {
            ...
            source: {
              ...
              data: part.imageUrl, // Assuming base64 data
            },
          };
        }
        return part;
  ...
  getCostInDollars(): number {
    return this.cost / 100; // Convert cents to dollars
  }
  ...
  getTokensPerSecond(): number {
    ...
    return (this.getTotalTokens() / this.responseTime) * 1000; // tokens per second
  }
