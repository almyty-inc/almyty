...
describe('ComplianceController entitlement gate', () => {
  ...
  it('blocks with 402 in the community edition', () => {
    ...
    try {
      guard.canActivate(ctxFor(ComplianceController, 'getPolicy'));
      fail('expected 402');
    } catch (e) {
  ...
  it('allows when compliance_pack is licensed', () => {
    const svc = new LicenseService();
    jest.spyOn(svc, 'has').mockImplementation((f) => f === EE_ENTITLEMENTS.COMPLIANCE_PACK);
    const guard = new EntitlementGuard(reflector, svc);
    expect(guard.canActivate(ctxFor(ComplianceController, 'getReport'))).toBe(true);
  });
});
